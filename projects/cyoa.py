...
def ferris_wheel() -> int:
    ...
    choice: str = input(f"{player}, Would you like to... one: Wait in line for the ride. or two: choose another option? Please enter your choice in all lowercase with no spaces or colons.")
    if choice == "one":
...
def funnel_cake(points: int) -> int:
    funnel_points: int = 0
    bogo: str = input(f"Good News! You received a buy one get one funnel cake coupon upon ticket purchase, {player}! Would you like to... one: Keep the free funnel cake for yoursef. or two: Give it to the person in line behind you? ")
    if bogo == "one":
...
def main() -> None:
    ...
    path: str = ""
    while path != "four":
        path = input(f"{player}, would you like to... one: Ride the Ferris Wheel. two: Go to the Tarot Card Booth. three: Get funnel cake. or four: Quit game. Type the option you are selecting in lowercase with no spaces or colons please! ")
        if path == "one":
            ferris_wheel()
        else:
            if path == "two":
